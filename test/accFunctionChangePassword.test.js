...
test("Test 3: new password empty",async () =>{
    let res =await user1.post(domain+"/account/password").send({curPassword:"password"});
    let text =res.text;
    expect(isJson(text)).toBeTruthy();
    let obj  = JSON.parse(text);
    expect(obj.success).toBeFalsy();
...
test("Test 4: new password validation",async () =>{
    let res = await user1.post(domain+"/account/password").send({curPassword:"password", newPassword:1234});
    let text = res.text
    expect(isJson(text)).toBeTruthy();
    let obj  = JSON.parse(text);
    expect(obj.success).toBeFalsy();
...
test("Test 6: change password",async () =>{
    let res = await user1.post(domain+"/account/password").send({curPassword:"password", newPassword:"password"});
    let text = res.text
    expect(isJson(text)).toBeTruthy();
    let obj  = JSON.parse(text);
